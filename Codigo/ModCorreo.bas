...
If ReceptIndex > 0 And UserList(UserIndex).flags.BattleModo = 0 Then
    ...
    If Index >= 1 And Index <= MAX_CORREOS_SLOTS Then
       ...
       ' Call WriteCorreoUpdateCount(ReceptIndex, UserList(ReceptIndex).Correo.MensajesSinLeer)
        ...
        If UserIndex <> 0 Then _
            Call WriteConsoleMsg(UserIndex, "Mensaje enviado.", FontTypeNames.FONTTYPE_INFOIAO)
            
        AddCorreo = True
        Exit Function
    Else
        If UserIndex <> 0 Then _
            Call WriteConsoleMsg(UserIndex, "No hay mas espacio para correos.", FontTypeNames.FONTTYPE_INFOIAO)
...
Else
    ...
    Else
        If UserIndex <> 0 Then _
            Call WriteConsoleMsg(UserIndex, "El personaje no existe.", FontTypeNames.FONTTYPE_INFOIAO) 'El personaje no existe o se encuentra baneado.
...
If UserList(UserIndex).Correo.Mensaje(Index).ItemCount <= 0 Then _
    Exit Sub
