...
End Function
Function DescripcionPalabraValida(ByVal cad As String) As Boolean
    On Error GoTo DescripcionPalabraValida_Err
    ...
    For i = LBound(PalabrasBloqueadasDesc) To UBound(PalabrasBloqueadasDesc)
        If InStr(1, cad, " " & PalabrasBloqueadasDesc(i) & " ", vbTextCompare) > 0 Then
            DescripcionPalabraValida = False
            Exit Function
        End If
    Next i

    DescripcionPalabraValida = True
    Exit Function

DescripcionPalabraValida_Err:
    Call TraceError(Err.Number, Err.Description, "TCP.DescripcionPalabraValida", Erl)
End Function
Private Function NormalizarTexto(ByVal cad As String) As String
On Error GoTo NormalizarTexto_Err
    ...
    For i = 1 To Len(signos)
        cad = Replace(cad, mid$(signos, i, 1), " ")
    Next i
    ...
    NormalizarTexto = Trim$(cad)
    Exit Function

NormalizarTexto_Err:
    Call TraceError(Err.Number, Err.Description, "TCP.NormalizarTexto", Erl)
End Function
...
112                     ValidarCabeza = head >= 200 And head <= 229 Or head >= 792 And head <= 810
