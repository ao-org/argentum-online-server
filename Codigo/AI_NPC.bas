...
Public Function SelectNearestUser(ByVal NpcIndex As Integer, _
                                  ByRef NearestTargetDistance As Single) As Integer
    ...
    With NpcList(NpcIndex)
        ...
        For i = 1 To ModAreas.ConnGroups(.pos.Map).CountEntrys
            ...
            ' Find nearest user
            If UserMod.IsVisible(UserList(UserIndex)) And UserCanAttackNpc(UserIndex, NpcIndex).CanAttack And NPCs.CanAttackUser(NpcIndex, UserIndex) = e_AttackInteractionResult.eCanAttack Then
...
Public Function SelectNearestNpc(ByVal NpcIndex, _
                                 ByRef NearestTargetDistance As Single) As Integer
...
Public Function SelectCurrentTarget(ByVal NpcIndex, _
                                    ByVal NearestUser As Integer) As t_AnyReference
...
108         AtacaMelee = EstaPegadoAlUsuario And UsuarioAtacableConMelee(AtackerNpcIndex, .TargetUser.ArrayIndex)
            AtacaMelee = AtacaMelee And (.flags.LanzaSpells > 0 And ((UserList(.TargetUser.ArrayIndex).flags.invisible > 0 Or UserList(.TargetUser.ArrayIndex).flags.Oculto > 0)) Or (IsFeatureEnabled("Magic_and_Punch") And Not IsSet(.flags.BehaviorFlags, e_BehaviorFlags.eDontHitVisiblePlayers)))
...
Private Function DistanciaRadial(OrigenPos As t_WorldPos, _
                                 DestinoPos As t_WorldPos) As Long
...
Public Sub AI_NpcAtacaNpc(ByVal NpcIndex As Integer, _
                          Optional ByVal ChangeTargetMovement As Boolean = True)
        On Error GoTo ErrorHandler
...
108                 If UserList(.MaestroUser.ArrayIndex).flags.Muerto = 0 And UserList(.MaestroUser.ArrayIndex).flags.invisible = 0 And UserList(.MaestroUser.ArrayIndex).flags.Oculto = 0 And Distancia(.pos, UserList(.MaestroUser.ArrayIndex).pos) > 3 Then
...
110     EnRangoVision = ((userPos.Map = NpcPos.Map) And (Abs(userPos.x - NpcPos.x) <= Limite_X) And (Abs(userPos.y - NpcPos.y) <= Limite_Y))
        Exit Function
EnRangoVision_Err:
...
104         UsuarioAtacableConMagia = (.flags.Muerto = 0 And .flags.invisible = 0 And .flags.Inmunidad = 0 And .flags.Oculto = 0 And .flags.Mimetizado < e_EstadoMimetismo.FormaBichoSinProteccion And Not (EsGM(targetUserIndex) And Not UserList(targetUserIndex).flags.AdminPerseguible) And Not .flags.EnConsulta)
...
106         UsuarioAtacableConMelee = (.flags.Muerto = 0 And .flags.Inmunidad = 0 And (EstaPegadoAlUser Or (Not EstaPegadoAlUser And (.flags.invisible + .flags.Oculto) = 0)) And .flags.Mimetizado < e_EstadoMimetismo.FormaBichoSinProteccion And Not (EsGM(targetUserIndex) And Not UserList(targetUserIndex).flags.AdminPerseguible) And Not .flags.EnConsulta)
...
Private Function SelectSupportSpellAndTarget(ByVal NpcIndex As Integer, _
                                             ByRef Target As t_AnyReference, _
                                             ByVal AvailableSpellEffect As Long) As Integer
        ...
        If IsSet(AvailableSpellEffect, e_SpellEffects.RemoveParalysis) Then
            If IsSet(.flags.BehaviorFlags, e_BehaviorFlags.eHelpUsers) Then
                ...
                For i = 1 To ModAreas.ConnGroups(.pos.Map).CountEntrys
                    ...
                    ' Find nearest user
                    If CanSeeUser(NpcIndex, TargetIndex) And NPCs.CanHelpUser(NpcIndex, TargetIndex) = eInteractionOk And (UserList(TargetIndex).flags.Inmovilizado Or UserList(TargetIndex).flags.Paralizado) Then
        ...
        If IsSet(AvailableSpellEffect, e_SpellEffects.eDoHeal) Then
            If IsSet(.flags.BehaviorFlags, e_BehaviorFlags.eHelpUsers) Then
                ...
                For i = 1 To ModAreas.ConnGroups(.pos.Map).CountEntrys
                    TargetIndex = ModAreas.ConnGroups(.pos.Map).UserEntrys(i)
                    ' Find nearest user
                    If CanSeeUser(NpcIndex, TargetIndex) And UserList(TargetIndex).Stats.MinHp < UserList(TargetIndex).Stats.MaxHp And NPCs.CanHelpUser(NpcIndex, TargetIndex) = eInteractionOk Then
                        UserDistance = Distance(UserList(TargetIndex).pos.x, UserList(TargetIndex).pos.y, .pos.x, .pos.y)
                        If UserDistance < .SpellRange Then
...
Private Function SelectAttackSpellAndTarget(ByVal NpcIndex As Integer, _
                                            ByRef Target As t_AnyReference, _
                                            ByVal AvailableSpellEffect As Long) As Integer
    ...
    With NpcList(NpcIndex)
        ...
        If IsSet(AvailableSpellEffect, e_SpellEffects.Paralize Or e_SpellEffects.Immobilize) Then
            ...
            If IsValidRef(Target) Then
                ...
                For SpellIndex = 1 To .flags.LanzaSpells
                    If CanCastSpell(NpcList(NpcIndex), SpellIndex) And IsSet(Hechizos(.Spells(SpellIndex).SpellIndex).Effects, e_SpellEffects.Paralize Or e_SpellEffects.Immobilize) Then
...
Public Function TryCastHelpSpell(ByVal NpcIndex As Integer, _
                                 ByVal AvailableSpellEffect As Long) As Boolean
...
Public Function TryCastAttackSpell(ByVal NpcIndex As Integer, _
                                   ByVal AvailableSpellEffect As Long) As Boolean
