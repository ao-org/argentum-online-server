    ...
    If Not (Distance(UserList(UserIndex).Pos.x, UserList(UserIndex).Pos.Y, x, Y) > 2) Then
        If ObjData(MapData(Map, x, Y).ObjInfo.ObjIndex).Llave = 0 Then
            ...
            Else
                ...
                'Si hay user en la puerta lo corre un tile mas abajo. Ladder
                '  If MapData(Map, X, Y).UserIndex <> 0 Then
                '    Call FindLegalPos(UserIndex, Map, X, Y + 1)
                '      Call WarpUserChar(MapData(Map, X, Y).UserIndex, Map, X, Y + 1, False)
                '   End If
                
                '   If MapData(Map, X - 1, Y).UserIndex <> 0 Then
                '      Call FindLegalPos(UserIndex, Map, X - 1, Y + 1)
                '       Call WarpUserChar(MapData(Map, X - 1, Y).UserIndex, Map, X - 1, Y + 1, False)
                '   End If
...
Sub AccionParaRamita(ByVal Map As Integer, ByVal x As Integer, ByVal Y As Integer, ByVal UserIndex As Integer)
    ...
    If MapData(Map, x - 1, Y).ObjInfo.ObjIndex = FOGATA Or MapData(Map, x + 1, Y).ObjInfo.ObjIndex = FOGATA Or MapData(Map, x, Y - 1).ObjInfo.ObjIndex = FOGATA Or MapData(Map, x, Y + 1).ObjInfo.ObjIndex = FOGATA Then
    ...
    If exito = 1 Then
        If MapInfo(UserList(UserIndex).Pos.Map).zone <> Ciudad Then
            obj.ObjIndex = FOGATA
            obj.Amount = 1
