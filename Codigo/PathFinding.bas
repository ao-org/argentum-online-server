...
Public Function NpcHasActiveStrafe(ByVal NpcIndex As Integer) As Boolean
    On Error GoTo NpcHasActiveStrafe_Err
    If NpcIndex <= 0 Or NpcIndex > UBound(NpcList) Then Exit Function

    With NpcList(NpcIndex).pathFindingInfo
        If .StrafeExpiresAt = 0 Then Exit Function
        If TickAfter(GlobalFrameTime, .StrafeExpiresAt) Then
            Call ResetNpcStrafeInfo(NpcList(NpcIndex).pathFindingInfo)
            Exit Function
        End If

        If .StrafeOffset.x <> 0 Or .StrafeOffset.y <> 0 Then
            NpcHasActiveStrafe = True
        End If
    End With

    Exit Function
NpcHasActiveStrafe_Err:
    Call TraceError(Err.Number, Err.Description, "PathFinding.NpcHasActiveStrafe", Erl)
End Function
...
Public Sub RegisterNpcDamageStrafe(ByVal NpcIndex As Integer, ByVal SourceIndex As Integer, ByVal SourceType As e_ReferenceType)
    ...
    Dim haveAttacker As Boolean
    Select Case SourceType
        Case e_ReferenceType.eUser
            If SourceIndex > 0 And SourceIndex <= UBound(UserList) Then
                If UserList(SourceIndex).flags.Muerto = 0 Then
                    attackerPos = UserList(SourceIndex).pos
                    haveAttacker = True
                End If
            End If
        Case e_ReferenceType.eNpc
            If SourceIndex > 0 And SourceIndex <= UBound(NpcList) Then
                If NpcList(SourceIndex).flags.NPCActive <> 0 Then
    ...
    If haveAttacker Then
        ...
        Exit Sub
    End If
    ' No direct attacker reference (environmental damage, traps, etc.).
    ' Try to pivot around the current combat target so the NPC still reacts.
    Dim targetPos As t_WorldPos
    If IsValidUserRef(NpcList(NpcIndex).TargetUser) Then
        With UserList(NpcList(NpcIndex).TargetUser.ArrayIndex)
            If .flags.Muerto = 0 Then
                targetPos = .pos
                If targetPos.Map = NpcList(NpcIndex).pos.Map Then
                    Call SetNpcStrafeOffsetFromAttacker(NpcIndex, targetPos.x, targetPos.y)
                    Exit Sub
                End If
            End If
        End With
    ...
    If IsValidNpcRef(NpcList(NpcIndex).TargetNPC) Then
        With NpcList(NpcList(NpcIndex).TargetNPC.ArrayIndex)
            If .flags.NPCActive <> 0 Then
                targetPos = .pos
                If targetPos.Map = NpcList(NpcIndex).pos.Map Then
                    Call SetNpcStrafeOffsetFromAttacker(NpcIndex, targetPos.x, targetPos.y)
                    Exit Sub
        ...
        End With
    End If
    ' As a last resort, try to pick a lateral move around the NPC's own tile.
    Call SetNpcSelfStrafeOffset(NpcIndex)
    Exit Sub
RegisterNpcDamageStrafe_Err:
    Call TraceError(Err.Number, Err.Description, "PathFinding.RegisterNpcDamageStrafe", Erl)
End Sub

Private Sub SetNpcSelfStrafeOffset(ByVal NpcIndex As Integer)
    On Error GoTo SetNpcSelfStrafeOffset_Err
    If NpcIndex <= 0 Or NpcIndex > UBound(NpcList) Then Exit Sub
    With NpcList(NpcIndex)
        ...
        Dim offsets(1 To 4) As t_Position
        offsets(1).x = 1: offsets(1).y = 0
        offsets(2).x = -1: offsets(2).y = 0
        offsets(3).x = 0: offsets(3).y = 1
        offsets(4).x = 0: offsets(4).y = -1
        Dim legalOffsets(1 To 4) As t_Position
        Dim legalCount As Integer
        Dim idx As Integer
        For idx = 1 To 4
            Dim candidateX As Integer
            Dim candidateY As Integer
            candidateX = .pos.x + offsets(idx).x
            candidateY = .pos.y + offsets(idx).y
            If LegalPos(.pos.Map, candidateX, candidateY, canWalkWater, canWalkGround) Then
                legalCount = legalCount + 1
                legalOffsets(legalCount) = offsets(idx)
            End If
        Next idx
        ...
        If legalCount = 1 Then
            choice = 1
        Else
            choice = RandomNumber(1, legalCount)
        End If
        With .pathFindingInfo
            .StrafeOffset.x = legalOffsets(choice).x
            .StrafeOffset.y = legalOffsets(choice).y
    ...
    End With
    Exit Sub
SetNpcSelfStrafeOffset_Err:
    Call TraceError(Err.Number, Err.Description, "PathFinding.SetNpcSelfStrafeOffset", Erl)
End Sub
