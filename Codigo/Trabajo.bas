...
Else
    ...
    If UserList(UserIndex).flags.Muerto = 0 Then
        ...
        If UserList(UserIndex).Invent.WeaponEqpObjIndex > 0 Then _
            UserList(UserIndex).Char.WeaponAnim = ObjData(UserList(UserIndex).Invent.WeaponEqpObjIndex).WeaponAnim
        If UserList(UserIndex).Invent.NudilloObjIndex > 0 Then _
            UserList(UserIndex).Char.WeaponAnim = ObjData(UserList(UserIndex).Invent.NudilloObjIndex).WeaponAnim
        If UserList(UserIndex).Invent.HerramientaEqpObjIndex > 0 Then _
            UserList(UserIndex).Char.WeaponAnim = ObjData(UserList(UserIndex).Invent.HerramientaEqpObjIndex).WeaponAnim
...
PuedeConstruir = HerreroTieneMateriales(UserIndex, ItemIndex) And UserList(UserIndex).Stats.UserSkills(eSkill.Manualidades) >= _
...
If CarpinteroTieneMateriales(UserIndex, ItemIndex) And _
   UserList(UserIndex).Stats.UserSkills(eSkill.Manualidades) >= _
...
If AlquimistaTieneMateriales(UserIndex, ItemIndex) And _
   ...
   UserList(UserIndex).Invent.HerramientaEqpObjIndex = OLLA_ALQUIMIA Then
...
If SastreTieneMateriales(UserIndex, ItemIndex) And _
   UserList(UserIndex).Stats.UserSkills(eSkill.Manualidades) >= _
   ObjData(ItemIndex).SkMAGOria And _
   PuedeConstruirSastre(ItemIndex) And _
   UserList(UserIndex).Invent.HerramientaEqpObjIndex = COSTURERO Then
    UserList(UserIndex).Stats.MinSta = UserList(UserIndex).Stats.MinSta - 2
    Call WriteUpdateSta(UserIndex)
...
'    Call LogTarea("Sub DoLingotes")
    ...
    If UserList(UserIndex).Invent.Object(slot).Amount < MineralesParaLingote(obji, cant) Or _
...
ObjData(OI).donador = 0 And _
ObjData(OI).OBJType <> eOBJType.otBarcos And _
ObjData(OI).OBJType <> eOBJType.otRunas And _
ObjData(OI).Instransferible = 0 And _
ObjData(OI).OBJType <> eOBJType.otMonturas
...
If RandomNumber(0, 70) < Suerte Then
    If VictimUserIndex <> 0 Then
        ...
        UserList(VictimUserIndex).Stats.MinHp = UserList(VictimUserIndex).Stats.MinHp - daño
            ...
            Call WriteEfectOverHead(UserIndex, daño, UserList(UserIndex).Char.CharIndex, &HFF8000) 'LADDER 21.11.08
        ...
        End If
         ...
         Call WriteEfectOverHead(VictimUserIndex, daño, UserList(UserIndex).Char.CharIndex) 'LADDER 21.11.08
