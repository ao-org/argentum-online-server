...
118     Call TraceError(Err.Number, Err.Description + " UI: " & UserIndex & " Slot:" & Slot, "modBanco.UpdateBanUserInv " & "userID: " & UserList(UserIndex).ID & " Slot: " & Slot & " Caller: " & caller, Erl)
    ...
    Exit Sub
UpdateBanUserInv_Err:
    Call TraceError(Err.Number, Err.Description, "modBanco.UpdateBanUserInv", Erl)
End Sub
...
112     Call TraceError(Err.Number, Err.Description, "modBanco.UsaRetiraItem")
    ...
    Exit Sub
UserRetiraItem_Err:
    Call TraceError(Err.Number, Err.Description, "modBanco.UserRetiraItem", Erl)
End Sub
...
UserDepositaItem_Err:
    Call TraceError(Err.Number, Err.Description, "modBanco.UserDepositaItem", Erl)
End Sub
...
102     Call WriteConsoleMsg(sendIndex, PrepareMessageLocaleMsg(1939, UserList(UserIndex).BancoInvent.NroItems, e_FontTypeNames.FONTTYPE_INFO)) ' Msg1939= Tiene ¬1 objetos.

104     For j = 1 To MAX_BANCOINVENTORY_SLOTS

106         If UserList(UserIndex).BancoInvent.Object(j).ObjIndex > 0 Then
108             Call WriteConsoleMsg(sendIndex, PrepareMessageLocaleMsg(1940, j & "¬" & ObjData(UserList(UserIndex).BancoInvent.Object(j).ObjIndex).name & "¬" & UserList(UserIndex).BancoInvent.Object(j).amount, e_FontTypeNames.FONTTYPE_INFO)) ' Msg1940= Objeto ¬1 ¬2 Cantidad:¬3
            End If
