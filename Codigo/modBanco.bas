    ...
    If slotvalido = False Then
        ...
        If Slot > UserList(UserIndex).CurrentInventorySlots Then
            ...
            Do Until UserList(UserIndex).invent.Object(Slot).ObjIndex = 0
                Slot = Slot + 1
                If Slot > UserList(UserIndex).CurrentInventorySlots Then
                    Call WriteLocaleMsg(UserIndex, MSG_NO_PODES_TENER_MAS_OBJETOS, e_TextChannel.TEXTCHANNEL_ECONOMY, e_FontTypeNames.FONTTYPE_New_Naranja) 'Msg1600= No podés tener más objetos.
                    Exit Function
                End If
            Loop
...
Sub UserDepositaItem(ByVal UserIndex As Integer, ByVal Slot As Integer, ByVal Cantidad As Integer, ByVal slotdestino As Integer)
    ...
    If UserList(UserIndex).invent.Object(Slot).amount > 0 And Cantidad > 0 Then
        If ObjData(UserList(UserIndex).invent.Object(Slot).ObjIndex).OBJType = e_OBJType.otWildMount Then
            Call WriteLocaleMsg(UserIndex, MSG_WILD_MOUNT_CANNOT_BE_BANKED, e_TextChannel.TEXTCHANNEL_ECONOMY, e_FontTypeNames.FONTTYPE_New_Naranja)
            Exit Sub
        End If
...
Function UserDejaObj(ByVal UserIndex As Integer, ByVal ObjIndex As Integer, ByVal Cantidad As Integer, ByVal slotdestino As Integer) As Long
    ...
    If slotvalido = False Then
        ...
        If Slot > MAX_BANCOINVENTORY_SLOTS Then
            ...
            Do Until UserList(UserIndex).BancoInvent.Object(Slot).ObjIndex = 0
                Slot = Slot + 1
                If Slot > MAX_BANCOINVENTORY_SLOTS Then
                    Call WriteLocaleMsg(UserIndex, MSG_NO_TIENES_MAS_ESPACIO_BANCO, e_TextChannel.TEXTCHANNEL_ECONOMY, e_FontTypeNames.FONTTYPE_New_Naranja) 'Msg1601= No tienes más espacio en el banco.
                    Exit Function
                End If
            Loop
            UserList(UserIndex).BancoInvent.NroItems = UserList(UserIndex).BancoInvent.NroItems + 1
    ...
    If Slot <= MAX_BANCOINVENTORY_SLOTS Then 'Slot valido
        ...
        Else
            Call WriteLocaleMsg(UserIndex, MSG_NO_BANCO_PUEDE_CARGAR_TANTOS_OBJETOS, e_TextChannel.TEXTCHANNEL_ECONOMY, e_FontTypeNames.FONTTYPE_New_Naranja) 'Msg1602= El banco no puede cargar tantos objetos.
        End If
    End If
    UserDejaObj = Slot
    Exit Function
...
Sub SendUserBovedaTxt(ByVal sendIndex As Integer, ByVal UserIndex As Integer)
    ...
    Dim j As Integer
    Call WriteConsoleMsg(sendIndex, UserList(UserIndex).name, e_TextChannel.TEXTCHANNEL_ECONOMY, e_FontTypeNames.FONTTYPE_SUBASTA)
    Call WriteLocaleMsg(sendIndex, MSG_BANK_ITEM_COUNT, e_TextChannel.TEXTCHANNEL_ECONOMY, e_FontTypeNames.FONTTYPE_SUBASTA, UserList(UserIndex).BancoInvent.NroItems) ' Msg1939= Tiene ¬1 objetos.
    For j = 1 To MAX_BANCOINVENTORY_SLOTS
        If UserList(UserIndex).BancoInvent.Object(j).ObjIndex > 0 Then
            Call WriteLocaleMsg(sendIndex, MSG_BANK_ITEM_ENTRY, e_TextChannel.TEXTCHANNEL_ECONOMY, e_FontTypeNames.FONTTYPE_SUBASTA, j & "¬" & ObjData(UserList(UserIndex).BancoInvent.Object(j).ObjIndex).name & "¬" & UserList( _
                    UserIndex).BancoInvent.Object(j).amount) ' Msg1940= Objeto ¬1 ¬2 Cantidad:¬3
        End If
    Next
    Exit Sub
