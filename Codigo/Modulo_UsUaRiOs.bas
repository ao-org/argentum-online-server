...
HayError:
        ...
        Dim Desc As String
144     Desc = Err.Description & vbNewLine & " Usuario: " & UserList(UserIndex).name & vbNewLine & "Pos: " & Map & "-" & x & "-" & y
                                                    ...
                                                    If Not CheckGuildSend(UserList(UserIndex), UserList(tempIndex)) Then
                                                        If .Counters.timeFx + .Counters.timeChat = 0 Then
                                                            If Distancia(nPos, UserList(tempIndex).pos) > DISTANCIA_ENVIO_DATOS Then
                                                                ...
                                                                Call WritePlayWaveStep(tempIndex, .Char.charindex, MapData(nPos.Map, nPos.x, nPos.y).Graphic(1), MapData(nPos.Map, nPos.x, nPos.y).Graphic(2), Distance(nPos.x, nPos.y, UserList(tempIndex).pos.x, UserList(tempIndex).pos.y), Sgn(nPos.x - UserList(tempIndex).pos.x), .flags.stepToggle)
                                                            Else
...
142         CanAttackUser = eMounted
        ...
        If Not MapInfo(UserList(TargetIndex).pos.Map).FriendlyFire And UserList(TargetIndex).flags.CurrentTeam > 0 And UserList(TargetIndex).flags.CurrentTeam = UserList(attackerIndex).flags.CurrentTeam Then
