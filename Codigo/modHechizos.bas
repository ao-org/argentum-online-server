...
202                     Call SendData(SendTarget.ToNPCAliveArea, TargetNPC, PrepareMessageParticleFX(.Char.charindex, Hechizos(Spell).Particle, Hechizos(Spell).TimeParticula, _
                                False))
                    End If
...
100         Call SendData(SendTarget.ToPCAliveArea, UserIndex, PrepareMessageChatOverHead("PMAG*" & Hechizo, UserList(UserIndex).Char.charindex, vbCyan, True, UserList( _
                    UserIndex).pos.x, UserList(UserIndex).pos.y, RequiredSpellDisplayTime, MaxInvisibleSpellDisplayTime))
        Else
102         Call SendData(SendTarget.ToPCAliveArea, UserIndex, PrepareMessageChatOverHead("PMAG*" & Hechizo, UserList(UserIndex).Char.charindex, vbCyan, True, UserList( _
...
118         If .flags.Montado = 1 Then
            ...
            If Hechizos(HechizoIndex).NecesitaObj > 0 Then
                If Not IsObjecIndextInInventory(UserIndex, Hechizos(HechizoIndex).NecesitaObj) And Not IsObjecIndextInInventory(UserIndex, Hechizos(HechizoIndex).NecesitaObj2) Then
                    If Not IsObjecIndextInInventory(UserIndex, Hechizos(HechizoIndex).NecesitaObj) And Not IsObjecIndextInInventory(UserIndex, Hechizos( _
                            HechizoIndex).NecesitaObj2) Then
                        ...
                        If .MascotasIndex(j).ArrayIndex > 0 Then
                            If IsValidNpcRef(.MascotasIndex(j)) Then
                                If NpcList(.MascotasIndex(j).ArrayIndex).flags.NPCActive Then
                                    If NpcList(.MascotasIndex(j).ArrayIndex).Contadores.TiempoExistencia > 0 And NpcList(.MascotasIndex( _
                                            j).ArrayIndex).Contadores.TiempoExistencia < MinTiempo Then
                                        Index = j
...
118                     If MapData(PosCasteadaM, TempX, TempY).UserIndex > 0 Then
                            'hay un user
120                         If UserList(MapData(PosCasteadaM, TempX, TempY).UserIndex).flags.invisible = 1 And UserList(MapData(PosCasteadaM, TempX, _
                                    TempY).UserIndex).flags.NoDetectable = 0 Then
122                             UserList(MapData(PosCasteadaM, TempX, TempY).UserIndex).flags.invisible = 0
124                             Call WriteConsoleMsg(MapData(PosCasteadaM, TempX, TempY).UserIndex, PrepareMessageLocaleMsg(1869, vbNullString, _
                                        e_FontTypeNames.FONTTYPE_INFOIAO)) ' Msg1869=Tu invisibilidad ya no tiene efecto.
126                             Call SendData(SendTarget.ToPCArea, UserIndex, PrepareMessageSetInvisible(UserList(MapData(PosCasteadaM, TempX, TempY).UserIndex).Char.charindex, _
...
1114                            Call SendData(SendTarget.ToPCArea, UserIndex, PrepareMessageParticleFXWithDestinoXY(UserList(UserIndex).Char.charindex, Hechizos( _
                                        h).ParticleViaje, Hechizos(h).FXgrh, 1, Hechizos(h).wav, 1, x, y))
                            Else
1116                            Call SendData(SendTarget.ToPCArea, UserIndex, PrepareMessageFxPiso(Hechizos(h).FXgrh, UserList(TargetMap.UserIndex).pos.x, UserList( _
...
11114                           Call SendData(SendTarget.ToPCArea, UserIndex, PrepareMessageParticleFXWithDestinoXY(UserList(UserIndex).Char.charindex, Hechizos( _
                                        h).ParticleViaje, Hechizos(h).FXgrh, 1, Hechizos(h).wav, 1, x, y))
                            Else
11116                           Call SendData(SendTarget.ToPCArea, UserIndex, PrepareMessageFxPiso(Hechizos(h).FXgrh, NpcList(TargetMap.NpcIndex).pos.x, NpcList( _
...
106     uh = UserList(UserIndex).Stats.UserHechizos(UserList(UserIndex).flags.Hechizo)
        'Envio Palabras magicas, wavs y fxs.
108     If MapData(UserList(UserIndex).pos.Map, UserList(UserIndex).flags.TargetX, UserList(UserIndex).flags.TargetY).ObjInfo.amount > 0 Or (MapData(UserList(UserIndex).pos.Map, _
...
Sub HechizoMaterializacion(ByVal UserIndex As Integer, ByRef b As Boolean)
        ...
        Dim MAT As t_Obj
100     h = UserList(UserIndex).Stats.UserHechizos(UserList(UserIndex).flags.Hechizo)
102     If MapData(UserList(UserIndex).pos.Map, UserList(UserIndex).flags.TargetX, UserList(UserIndex).flags.TargetY).ObjInfo.amount > 0 Or MapData(UserList(UserIndex).pos.Map, _
...
114         Call SendData(SendTarget.ToPCArea, UserIndex, PrepareMessageParticleFXToFloor(UserList(UserIndex).flags.TargetX, UserList(UserIndex).flags.TargetY, Hechizos( _
...
100     h = UserList(UserIndex).Stats.UserHechizos(UserList(UserIndex).flags.Hechizo)
102     targetUserIndex = UserList(UserIndex).flags.TargetUser.ArrayIndex
104     If IsSet(Hechizos(h).Effects, e_SpellEffects.Invisibility) Then
106         If UserList(targetUserIndex).flags.Muerto = 1 Then
                'Msg77=¡¡Estás muerto!!.
...
124         If UserList(UserIndex).flags.Montado Then
                ...
                Exit Sub
            End If
125         If UserList(targetUserIndex).flags.Montado Then
                'Msg795= No podés lanzar invisibilidad a alguien montado.
                Call WriteLocaleMsg(UserIndex, "795", e_FontTypeNames.FONTTYPE_INFO)
                Exit Sub
            End If
128         If UserList(targetUserIndex).Counters.Saliendo Then
130             If UserIndex <> targetUserIndex Then
...
160             If Not UserList(targetUserIndex).flags.Privilegios And e_PlayerType.User Then
                    Exit Sub
                End If
            End If
162         If MapInfo(UserList(targetUserIndex).pos.Map).SinInviOcul Then
                ...
                Exit Sub
            End If
166         If UserList(targetUserIndex).flags.invisible = 1 Or UserList(targetUserIndex).Counters.DisabledInvisibility > 0 Then
168             If targetUserIndex = UserIndex Then
...
174             b = False
            ...
            If IsSet(UserList(targetUserIndex).flags.StatusMask, eTaunting) Then
                ...
                Exit Sub
            End If
176         UserList(targetUserIndex).flags.invisible = 1
            'Ladder
            ...
            If UserList(targetUserIndex).Counters.Invisibilidad <= 0 Then UserList(targetUserIndex).Counters.Invisibilidad = RandomNumber(Hechizos(h).Duration - 4, Hechizos( _
                    h).Duration + 1)
177         Call WriteContadores(targetUserIndex)
178         Call SendData(SendTarget.ToPCArea, targetUserIndex, PrepareMessageSetInvisible(UserList(targetUserIndex).Char.charindex, True, UserList(targetUserIndex).pos.x, _
...
190         If UserList(UserIndex).flags.EnReto Then
                ...
                Exit Sub
            End If
194         If UserList(targetUserIndex).flags.Muerto = 1 Then
                Exit Sub
            End If
196         If UserList(targetUserIndex).flags.Navegando = 1 Then
                Exit Sub
            End If
198         If UserList(UserIndex).flags.Navegando = 1 Then
            ...
            End If
            'Si sos user, no uses este hechizo con GMS.
200         If Not EsGM(UserIndex) And EsGM(targetUserIndex) Then Exit Sub
...
222             .Char.body = UserList(targetUserIndex).Char.body
224             .Char.head = UserList(targetUserIndex).Char.head
226             .Char.CascoAnim = UserList(targetUserIndex).Char.CascoAnim
228             .Char.ShieldAnim = UserList(targetUserIndex).Char.ShieldAnim
230             .Char.WeaponAnim = UserList(targetUserIndex).Char.WeaponAnim
231             .Char.CartAnim = UserList(targetUserIndex).Char.CartAnim
232             .NameMimetizado = UserList(targetUserIndex).name
234             If UserList(targetUserIndex).GuildIndex > 0 Then .NameMimetizado = .NameMimetizado & " <" & modGuilds.GuildName(UserList(targetUserIndex).GuildIndex) & ">"
...
246         If Not PuedeAtacar(UserIndex, targetUserIndex) Then Exit Sub
248         If UserList(targetUserIndex).flags.Envenenado = 0 Then
250             If UserIndex <> targetUserIndex Then
252                 Call UsuarioAtacadoPorUsuario(UserIndex, targetUserIndex)
                End If
254             UserList(targetUserIndex).flags.Envenenado = Hechizos(h).Envenena
256             UserList(targetUserIndex).Counters.Veneno = Hechizos(h).Duration
258             Call InfoHechizo(UserIndex)
260             b = True
            Else
262             Call WriteConsoleMsg(UserIndex, PrepareMessageLocaleMsg(1870, UserList(targetUserIndex).name, e_FontTypeNames.FONTTYPE_INFO)) ' Msg1870=¬1 ya está envenenado. El hechizo no tuvo efecto.
264             b = False
            End If
        End If
...
318         If UserIndex = targetUserIndex Then
320             Call WriteLocaleMsg(UserIndex, "380", e_FontTypeNames.FONTTYPE_FIGHT)
                Exit Sub
            End If
322         If Not PuedeAtacar(UserIndex, targetUserIndex) Then Exit Sub
324         If UserIndex <> targetUserIndex Then
326             Call UsuarioAtacadoPorUsuario(UserIndex, targetUserIndex)
            End If
328         UserList(targetUserIndex).flags.Incinerado = 1
...
336         If UserList(targetUserIndex).flags.Muerto = 1 Then
                'Msg77=¡¡Estás muerto!!.
...
342         If UserList(targetUserIndex).flags.Envenenado = 0 Then
344             Call WriteConsoleMsg(UserIndex, PrepareMessageLocaleMsg(1871, UserList(targetUserIndex).name, e_FontTypeNames.FONTTYPE_INFOIAO)) ' Msg1871=¬1 no está envenenado, el hechizo no tiene efecto.
346             b = False
                Exit Sub
            End If
            'Para poder tirar curar veneno a un pk en el ring
348         If Not PeleaSegura(UserIndex, targetUserIndex) Then
350             If Status(targetUserIndex) = 0 And Status(UserIndex) = 1 Or Status(targetUserIndex) = 2 And Status(UserIndex) = 1 Then
...
366             If Not UserList(targetUserIndex).flags.Privilegios And e_PlayerType.User Then
                    Exit Sub
                End If
            End If
368         UserList(targetUserIndex).flags.Envenenado = 0
370         UserList(targetUserIndex).Counters.Veneno = 0
372         Call InfoHechizo(UserIndex)
374         b = True
        End If
376     If IsSet(Hechizos(h).Effects, e_SpellEffects.Curse) Then
378         If UserIndex = targetUserIndex Then
380             Call WriteLocaleMsg(UserIndex, "380", e_FontTypeNames.FONTTYPE_FIGHT)
                Exit Sub
            End If
382         If Not PuedeAtacar(UserIndex, targetUserIndex) Then Exit Sub
384         If UserIndex <> targetUserIndex Then
386             Call UsuarioAtacadoPorUsuario(UserIndex, targetUserIndex)
            End If
388         UserList(targetUserIndex).flags.Maldicion = 1
390         UserList(targetUserIndex).Counters.Maldicion = 200
392         Call InfoHechizo(UserIndex)
394         b = True
        End If
396     If IsSet(Hechizos(h).Effects, e_SpellEffects.RemoveCurse) Then
398         UserList(targetUserIndex).flags.Maldicion = 0
400         UserList(targetUserIndex).Counters.Maldicion = 0
402         Call InfoHechizo(UserIndex)
404         b = True
        End If
406     If IsSet(Hechizos(h).Effects, e_SpellEffects.PreciseHit) Then
408         UserList(targetUserIndex).flags.GolpeCertero = 1
410         Call InfoHechizo(UserIndex)
412         b = True
        End If
422     If IsSet(Hechizos(h).Effects, e_SpellEffects.Paralize) Then
424         If UserIndex = targetUserIndex Then
...
428             ' Msg661=No podes inmovilizar un objetivo que no puede moverse.
            ...
            If IsSet(UserList(targetUserIndex).flags.StatusMask, eCCInmunity) Then
                ...
                Exit Sub
            End If
432         If Not PuedeAtacar(UserIndex, targetUserIndex) Then Exit Sub
434         If UserIndex <> targetUserIndex Then
                Call checkHechizosEfectividad(UserIndex, targetUserIndex)
436             Call UsuarioAtacadoPorUsuario(UserIndex, targetUserIndex)
            End If
...
440         b = True
            ...
            End If
444         If UserList(targetUserIndex).flags.Paralizado = 0 Then
446             UserList(targetUserIndex).flags.Paralizado = 1
448             Call WriteParalizeOK(targetUserIndex)
450             Call WritePosUpdate(targetUserIndex)
            End If
        End If
452     If Hechizos(h).velocidad <> 0 Then
            'Verificamos que el usuario no este muerto
454         If UserList(targetUserIndex).flags.Muerto = 1 Then
                'Msg77=¡¡Estás muerto!!.
...
462             If UserIndex = targetUserIndex Then
464                 Call WriteLocaleMsg(UserIndex, "380", e_FontTypeNames.FONTTYPE_FIGHT)
                    Exit Sub
                End If
466             If Not PuedeAtacar(UserIndex, targetUserIndex) Then Exit Sub
            Else
                'Para poder tirar curar veneno a un pk en el ring
468             If Not PeleaSegura(UserIndex, targetUserIndex) Then
470                 If Status(targetUserIndex) = 0 And Status(UserIndex) = 1 Or Status(targetUserIndex) = 2 And Status(UserIndex) = 1 Then
...
486                 If Not UserList(targetUserIndex).flags.Privilegios And e_PlayerType.User Then
                        Exit Sub
                    End If
                End If
            End If
488         Call UsuarioAtacadoPorUsuario(UserIndex, targetUserIndex)
490         Call InfoHechizo(UserIndex)
492         b = True
494         If UserList(targetUserIndex).Counters.velocidad = 0 Then
496             UserList(targetUserIndex).flags.VelocidadHechizada = Hechizos(h).velocidad
498             Call ActualizarVelocidadDeUsuario(targetUserIndex)
            End If
500         UserList(targetUserIndex).Counters.velocidad = Hechizos(h).Duration
        End If
502     If IsSet(Hechizos(h).Effects, e_SpellEffects.Immobilize) Then
504         If UserIndex = targetUserIndex Then
...
510             ' Msg661=No podes inmovilizar un objetivo que no puede moverse.
            ...
            End If
            If UserList(targetUserIndex).Counters.TiempoDeInmunidadParalisisNoMagicas > 0 Then
515             Call WriteConsoleMsg(UserIndex, PrepareMessageLocaleMsg(1873, UserList(targetUserIndex).name, e_FontTypeNames.FONTTYPE_FIGHT)) ' Msg1873=¬1 no puede volver a ser inmovilizado tan rápido.
                Exit Sub
            End If
            If IsSet(UserList(targetUserIndex).flags.StatusMask, eCCInmunity) Then
                Call WriteLocaleMsg(UserIndex, MsgCCInunity, e_FontTypeNames.FONTTYPE_FIGHT)
                Exit Sub
            End If
516         If Not PuedeAtacar(UserIndex, targetUserIndex) Then Exit Sub
518         If UserIndex <> targetUserIndex Then
                Call checkHechizosEfectividad(UserIndex, targetUserIndex)
520             Call UsuarioAtacadoPorUsuario(UserIndex, targetUserIndex)
            End If
...
524         b = True
            ...
            End If
528         UserList(targetUserIndex).flags.Inmovilizado = 1
530         Call WriteInmovilizaOK(targetUserIndex)
532         Call WritePosUpdate(targetUserIndex)
        End If
534     If IsSet(Hechizos(h).Effects, e_SpellEffects.RemoveParalysis) Then
            'Para poder tirar remo a un pk en el ring
536         If Not PeleaSegura(UserIndex, targetUserIndex) Then
                Select Case Status(UserIndex)
                    Case 1, 3, 5 'Ciudadano o armada
                        If Status(targetUserIndex) <> e_Facciones.Ciudadano And Status(targetUserIndex) <> e_Facciones.Armada And Status(targetUserIndex) <> e_Facciones.consejo _
                                Then
                ...
                End Select
            End If
554         If UserList(targetUserIndex).flags.Inmovilizado = 0 And UserList(targetUserIndex).flags.Paralizado = 0 Then
                'Msg806= El objetivo no esta paralizado.
                Call WriteLocaleMsg(UserIndex, "806", e_FontTypeNames.FONTTYPE_INFO)
558             b = False
                Exit Sub
            End If
560         If UserList(targetUserIndex).flags.Inmovilizado = 1 Then
562             UserList(targetUserIndex).Counters.Inmovilizado = 0
                ...
                End If
564             UserList(targetUserIndex).flags.Inmovilizado = 0
566             Call WriteInmovilizaOK(targetUserIndex)
568             Call WritePosUpdate(targetUserIndex)
            End If
570         If UserList(targetUserIndex).flags.Paralizado = 1 Then
572             UserList(targetUserIndex).flags.Paralizado = 0
                If UserList(targetUserIndex).clase = e_Class.Warrior Or UserList(targetUserIndex).clase = e_Class.Hunter Or UserList(targetUserIndex).clase = e_Class.Thief Or _
                        UserList(targetUserIndex).clase = e_Class.Pirat Then
                    UserList(targetUserIndex).Counters.TiempoDeInmunidadParalisisNoMagicas = 4
                End If
574             UserList(targetUserIndex).Counters.Paralisis = 0
576             Call WriteParalizeOK(targetUserIndex)
            End If
578         b = True
580         Call InfoHechizo(UserIndex)
        End If
582     If IsSet(Hechizos(h).Effects, e_SpellEffects.RemoveDumb) Then
584         If UserList(targetUserIndex).flags.Estupidez = 1 Then
                'Para poder tirar remo estu a un pk en el ring
586             If Not PeleaSegura(UserIndex, targetUserIndex) Then
588                 If Status(targetUserIndex) = 0 And Status(UserIndex) = 1 Or Status(targetUserIndex) = 2 And Status(UserIndex) = 1 Then
...
600                         b = False
                    ...
                    End If
                End If
602             UserList(targetUserIndex).flags.Estupidez = 0
604             UserList(targetUserIndex).Counters.Estupidez = 0
606             Call WriteDumbNoMore(targetUserIndex)
608             Call InfoHechizo(UserIndex)
610             b = True
            End If
        End If
612     If IsSet(Hechizos(h).Effects, e_SpellEffects.Resurrect) Then
614         If UserList(targetUserIndex).flags.Muerto = 1 Then
...
654             If Not PeleaSegura(UserIndex, targetUserIndex) Then
                    ...
                    Select Case Status(UserIndex)
                        Case 1, 3, 5 'Ciudadano o armada
                            If Status(targetUserIndex) <> e_Facciones.Ciudadano And Status(targetUserIndex) <> e_Facciones.Armada And Status(targetUserIndex) <> _
                            ...
                            End If
                        Case 2, 4 'Caos
                            If Status(targetUserIndex) <> e_Facciones.Caos And Status(targetUserIndex) <> e_Facciones.Criminal And Status(targetUserIndex) <> _
                ...
                Call SendData(SendTarget.ToPCArea, targetUserIndex, PrepareMessageSetInvisible(UserList(targetUserIndex).Char.charindex, False, UserList(targetUserIndex).pos.x, _
                        UserList(targetUserIndex).pos.y))
684             Call ResurrectUser(targetUserIndex, UserIndex)
...
694         If UserIndex = targetUserIndex Then
696             Call WriteLocaleMsg(UserIndex, "380", e_FontTypeNames.FONTTYPE_FIGHT)
                Exit Sub
            End If
698         If Not PuedeAtacar(UserIndex, targetUserIndex) Then Exit Sub
700         If UserIndex <> targetUserIndex Then
702             Call UsuarioAtacadoPorUsuario(UserIndex, targetUserIndex)
            End If
704         UserList(targetUserIndex).flags.Ceguera = 1
706         UserList(targetUserIndex).Counters.Ceguera = Hechizos(h).Duration
708         Call WriteBlind(targetUserIndex)
710         Call InfoHechizo(UserIndex)
712         b = True
        End If
714     If IsSet(Hechizos(h).Effects, e_SpellEffects.Dumb) Then
716         If UserIndex = targetUserIndex Then
718             Call WriteLocaleMsg(UserIndex, "380", e_FontTypeNames.FONTTYPE_FIGHT)
                Exit Sub
            End If
720         If Not PuedeAtacar(UserIndex, targetUserIndex) Then Exit Sub
722         If UserIndex <> targetUserIndex Then
724             Call UsuarioAtacadoPorUsuario(UserIndex, targetUserIndex)
            End If
726         If UserList(targetUserIndex).flags.Estupidez = 0 Then
728             UserList(targetUserIndex).flags.Estupidez = 1
730             UserList(targetUserIndex).Counters.Estupidez = Hechizos(h).Duration
            End If
732         Call WriteDumb(targetUserIndex)
...
192                 b = b Or ((esArmada(UserIndex) And (IsValidMaster And esArmada(.MaestroUser.ArrayIndex))) Or (esCaos(UserIndex) And (IsValidMaster And esCaos( _
...
104             If Hechizos(Spell).ParticleViaje > 0 Then
                    .Counters.timeFx = 3
106                 Call SendData(SendTarget.ToPCArea, TargetUser, PrepareMessageParticleFXWithDestino(NpcList(NpcIndex).Char.charindex, .Char.charindex, Hechizos( _
                ...
                Else
                    .Counters.timeFx = 3
108                 Call SendData(SendTarget.ToPCArea, TargetUser, PrepareMessageCreateFX(.Char.charindex, Hechizos(Spell).FXgrh, Hechizos(Spell).loops, UserList( _
                            TargetUser).pos.x, UserList(TargetUser).pos.y))
                End If
            End If
110         If Hechizos(Spell).Particle > 0 Then '¿Envio Particula?
112             If Hechizos(Spell).ParticleViaje > 0 Then
                    .Counters.timeFx = 3
114                 Call SendData(SendTarget.ToPCArea, TargetUser, PrepareMessageParticleFXWithDestino(NpcList(NpcIndex).Char.charindex, .Char.charindex, Hechizos( _
                ...
                Else
                    .Counters.timeFx = 3
116                 Call SendData(SendTarget.ToPCArea, TargetUser, PrepareMessageParticleFX(.Char.charindex, Hechizos(Spell).Particle, Hechizos(Spell).TimeParticula, False, , _
...
112                 Call SendData(SendTarget.ToPCAliveArea, UserList(UserIndex).flags.TargetUser.ArrayIndex, PrepareMessageParticleFXWithDestino(UserList( _
                            UserIndex).Char.charindex, UserList(UserList(UserIndex).flags.TargetUser.ArrayIndex).Char.charindex, Hechizos(h).ParticleViaje, Hechizos(h).FXgrh, _
                            Hechizos(h).TimeParticula, Hechizos(h).wav, 1, UserList(UserList(UserIndex).flags.TargetUser.ArrayIndex).pos.x, UserList(UserList( _
                            UserIndex).flags.TargetUser.ArrayIndex).pos.y))
                Else
                    UserList(UserList(UserIndex).flags.TargetUser.ArrayIndex).Counters.timeFx = 3
114                 Call SendData(SendTarget.ToPCAliveArea, UserList(UserIndex).flags.TargetUser.ArrayIndex, PrepareMessageCreateFX(UserList(UserList( _
                            UserIndex).flags.TargetUser.ArrayIndex).Char.charindex, Hechizos(h).FXgrh, Hechizos(h).loops, UserList(UserList( _
                            UserIndex).flags.TargetUser.ArrayIndex).pos.x, UserList(UserList(UserIndex).flags.TargetUser.ArrayIndex).pos.y))
                End If
            End If
116         If Hechizos(h).Particle > 0 Then '¿Envio Particula?
118             If Hechizos(h).ParticleViaje > 0 Then
                    UserList(UserList(UserIndex).flags.TargetUser.ArrayIndex).Counters.timeFx = 3
120                 Call SendData(SendTarget.ToPCAliveArea, UserList(UserIndex).flags.TargetUser.ArrayIndex, PrepareMessageParticleFXWithDestino(UserList( _
                            UserIndex).Char.charindex, UserList(UserList(UserIndex).flags.TargetUser.ArrayIndex).Char.charindex, Hechizos(h).ParticleViaje, Hechizos(h).Particle, _
                            Hechizos(h).TimeParticula, Hechizos(h).wav, 0, UserList(UserList(UserIndex).flags.TargetUser.ArrayIndex).pos.x, UserList(UserList( _
                            UserIndex).flags.TargetUser.ArrayIndex).pos.y))
                Else
                    UserList(UserList(UserIndex).flags.TargetUser.ArrayIndex).Counters.timeFx = 3
122                 Call SendData(SendTarget.ToPCAliveArea, UserList(UserIndex).flags.TargetUser.ArrayIndex, PrepareMessageParticleFX(UserList(UserList( _
                            UserIndex).flags.TargetUser.ArrayIndex).Char.charindex, Hechizos(h).Particle, Hechizos(h).TimeParticula, False, , UserList(UserList( _
                            UserIndex).flags.TargetUser.ArrayIndex).pos.x, UserList(UserList(UserIndex).flags.TargetUser.ArrayIndex).pos.y))
                End If
            End If
124         If Hechizos(h).ParticleViaje = 0 Then
126             Call SendData(SendTarget.ToPCAliveArea, UserList(UserIndex).flags.TargetUser.ArrayIndex, PrepareMessagePlayWave(Hechizos(h).wav, UserList(UserList( _
                        UserIndex).flags.TargetUser.ArrayIndex).pos.x, UserList(UserList(UserIndex).flags.TargetUser.ArrayIndex).pos.y))
            End If
...
140                     Call SendData(SendTarget.ToNPCAliveArea, UserList(UserIndex).flags.TargetNPC.ArrayIndex, PrepareMessageParticleFXWithDestinoXY(UserList( _
                                UserIndex).Char.charindex, Hechizos(h).ParticleViaje, Hechizos(h).FXgrh, Hechizos(h).TimeParticula, Hechizos(h).wav, 1, UserList( _
                                UserIndex).flags.TargetX, UserList(UserIndex).flags.TargetY))
                    Else
142                     Call SendData(SendTarget.ToNPCAliveArea, UserList(UserIndex).flags.TargetNPC.ArrayIndex, PrepareMessageFxPiso(Hechizos(h).FXgrh, UserList( _
                                UserIndex).flags.TargetX, UserList(UserIndex).flags.TargetY))
                    End If
                Else
144                 If Hechizos(h).ParticleViaje > 0 Then
146                     Call SendData(SendTarget.ToNPCAliveArea, UserList(UserIndex).flags.TargetNPC.ArrayIndex, PrepareMessageParticleFXWithDestino(UserList( _
                                UserIndex).Char.charindex, NpcList(UserList(UserIndex).flags.TargetNPC.ArrayIndex).Char.charindex, Hechizos(h).ParticleViaje, Hechizos(h).FXgrh, _
                                Hechizos(h).TimeParticula, Hechizos(h).wav, 1))
                    Else
148                     Call SendData(SendTarget.ToNPCAliveArea, UserList(UserIndex).flags.TargetNPC.ArrayIndex, PrepareMessageCreateFX(NpcList(UserList( _
                                UserIndex).flags.TargetNPC.ArrayIndex).Char.charindex, Hechizos(h).FXgrh, Hechizos(h).loops))
                    End If
                End If
            End If
...
154                 Call SendData(SendTarget.ToNPCAliveArea, UserList(UserIndex).flags.TargetNPC.ArrayIndex, PrepareMessageParticleFXWithDestinoXY(UserList( _
                            UserIndex).Char.charindex, Hechizos(h).ParticleViaje, Hechizos(h).Particle, Hechizos(h).TimeParticula, Hechizos(h).wav, 0, NpcList(UserList( _
...
158                     Call SendData(SendTarget.ToNPCAliveArea, UserList(UserIndex).flags.TargetNPC.ArrayIndex, PrepareMessageParticleFXWithDestino(UserList( _
                                UserIndex).Char.charindex, NpcList(UserList(UserIndex).flags.TargetNPC.ArrayIndex).Char.charindex, Hechizos(h).ParticleViaje, Hechizos( _
                                h).Particle, Hechizos(h).TimeParticula, Hechizos(h).wav, 0))
                    Else
160                     Call SendData(SendTarget.ToNPCAliveArea, UserList(UserIndex).flags.TargetNPC.ArrayIndex, PrepareMessageParticleFX(NpcList(UserList( _
                                UserIndex).flags.TargetNPC.ArrayIndex).Char.charindex, Hechizos(h).Particle, Hechizos(h).TimeParticula, False))
                    End If
                End If
            End If
162         If Hechizos(h).ParticleViaje = 0 Then
164             Call SendData(SendTarget.ToNPCAliveArea, UserList(UserIndex).flags.TargetNPC.ArrayIndex, PrepareMessagePlayWave(Hechizos(h).wav, NpcList(UserList( _
                        UserIndex).flags.TargetNPC.ArrayIndex).pos.x, NpcList(UserList(UserIndex).flags.TargetNPC.ArrayIndex).pos.y))
            End If
        Else ' Entonces debe ser sobre el terreno
166         If Hechizos(h).FXgrh > 0 Then 'Envio Fx?
168             Call modSendData.SendToAreaByPos(UserList(UserIndex).pos.Map, UserList(UserIndex).flags.TargetX, UserList(UserIndex).flags.TargetY, PrepareMessageFxPiso(Hechizos( _
...
172             Call SendData(SendTarget.ToPCAliveArea, UserIndex, PrepareMessageParticleFXToFloor(UserList(UserIndex).flags.TargetX, UserList(UserIndex).flags.TargetY, Hechizos( _
                        h).Particle, Hechizos(h).TimeParticula))
            End If
174         If Hechizos(h).wav <> 0 Then
176             Call SendData(SendTarget.ToPCAliveArea, UserIndex, PrepareMessagePlayWave(Hechizos(h).wav, UserList(UserIndex).flags.TargetX, UserList(UserIndex).flags.TargetY)) _
...
238         UserList(tempChr).Stats.UserAtributos(e_Atributos.Agilidad) = MinimoInt(UserList(tempChr).Stats.UserAtributos(e_Atributos.Agilidad) + Damage, UserList( _
...
294         UserList(tempChr).Stats.UserAtributos(e_Atributos.Fuerza) = MinimoInt(UserList(tempChr).Stats.UserAtributos(e_Atributos.Fuerza) + Damage, UserList( _
...
453         Call EffectsOverTime.TargetDidHit(UserList(UserIndex).EffectOverTime, tempChr, eUser, e_DamageSourceType.e_magic)
...
130         UserList(targetUserIndex).Stats.UserAtributos(e_Atributos.Agilidad) = MinimoInt(UserList(targetUserIndex).Stats.UserAtributos(e_Atributos.Agilidad) + Damage, _
                    UserList(targetUserIndex).Stats.UserAtributosBackUP(e_Atributos.Agilidad) * 2)
...
186         UserList(targetUserIndex).Stats.UserAtributos(e_Atributos.Fuerza) = MinimoInt(UserList(targetUserIndex).Stats.UserAtributos(e_Atributos.Fuerza) + Damage, UserList( _
...
321         Call EffectsOverTime.TargetDidHit(UserList(UserIndex).EffectOverTime, targetUserIndex, eUser, e_DamageSourceType.e_magic)
...
412         Call SendData(SendTarget.ToPCAliveArea, NpcIndex, PrepareMessageSetInvisible(UserList(NpcIndex).Char.charindex, True, UserList(NpcIndex).pos.x, UserList( _
                    NpcIndex).pos.y))
        End If
...
430             UserList(NpcIndex).flags.Inmovilizado = 0
                If UserList(NpcIndex).clase = e_Class.Warrior Or UserList(NpcIndex).clase = e_Class.Hunter Or UserList(NpcIndex).clase = e_Class.Thief Or UserList( _
                        NpcIndex).clase = e_Class.Pirat Then
