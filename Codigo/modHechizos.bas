...
350                 Call ActualizarVelocidadDeUsuario(UserIndex)
        ...
        With NpcList(NpcIndex)
            ...
            If IsSet(.flags.BehaviorFlags, e_BehaviorFlags.eDisplayCastMessage) Then
                Call SendData(SendTarget.ToNPCAliveArea, NpcIndex, PrepareMessageChatOverHead("PMAG*" & Spell, .Char.charindex, vbCyan, True, .pos.x, .pos.y, RequiredSpellDisplayTime, MaxInvisibleSpellDisplayTime))
            End If
...
Sub NpcLanzaSpellSobreNpc(ByVal NpcIndex As Integer, _
...
148             Next X
        ...
        With NpcList(NpcIndex)
            ...
            If IsSet(.flags.BehaviorFlags, e_BehaviorFlags.eDisplayCastMessage) Then
                Call SendData(SendTarget.ToNPCAliveArea, NpcIndex, PrepareMessageChatOverHead("PMAG*" & SpellIndex, .Char.charindex, vbCyan, True, .pos.x, .pos.y, RequiredSpellDisplayTime, MaxInvisibleSpellDisplayTime))
            End If
...
Private Function PuedeLanzar(ByVal UserIndex As Integer, _
                             ByVal HechizoIndex As Integer, _
                             Optional ByVal Slot As Integer = 0) As Boolean
...
118         If .flags.Montado = 1 Then
            ...
            ElseIf IsValidNpcRef(.flags.TargetNPC) Then
                ...
                If Hechizos(HechizoIndex).TargetEffectType = e_TargetEffectType.eNegative Then
                    ...
                    If UserAttackInteractionResult.Result = e_AttackInteractionResult.eAttackCitizenNpc Or UserAttackInteractionResult.Result = e_AttackInteractionResult.eRemoveSafeCitizenNpc Or UserAttackInteractionResult.Result = e_AttackInteractionResult.eSameFaction Or UserAttackInteractionResult.Result = e_AttackInteractionResult.eRemoveSafe Then
...
110             Call HechizoCombinados(UserIndex, b, IsAlive)
            ...
            Case e_TipoHechizo.uPhysicalSkill
                b = HandlePhysicalSkill(UserIndex, eUser, UserList(UserIndex).flags.TargetUser.ArrayIndex, eUser, UserList(UserIndex).Stats.UserHechizos(UserList(UserIndex).flags.Hechizo), IsAlive)
        End Select
...
Public Function GetSpellManaCostModifierByClass(ByVal UserIndex As Integer, _
                                                Hechizo As t_Hechizo, _
                                                Optional ByVal HechizoIndex As Long) As Integer
...
106             Call HechizoPropNPC(uh, UserList(UserIndex).flags.TargetNPC.ArrayIndex, UserIndex, b, IsAlive)
            ...
            Case e_TipoHechizo.uPhysicalSkill
                b = HandlePhysicalSkill(UserIndex, eUser, UserList(UserIndex).flags.TargetNPC.ArrayIndex, eNpc, UserList(UserIndex).Stats.UserHechizos(UserList(UserIndex).flags.Hechizo), IsAlive)
        End Select
...
176         UserList(targetUserIndex).flags.invisible = 1
            ...
            If UserList(targetUserIndex).Counters.Invisibilidad <= 0 Then UserList(targetUserIndex).Counters.Invisibilidad = RandomNumber(Hechizos(h).Duration - 4, Hechizos(h).Duration + 1)
177         Call WriteContadores(targetUserIndex)
178         Call SendData(SendTarget.ToPCArea, targetUserIndex, PrepareMessageSetInvisible(UserList(targetUserIndex).Char.charindex, True, UserList(targetUserIndex).pos.x, UserList(targetUserIndex).pos.y))
...
562             UserList(targetUserIndex).Counters.Inmovilizado = 0
                ...
                If UserList(targetUserIndex).clase = e_Class.Warrior Or UserList(targetUserIndex).clase = e_Class.Hunter Or UserList(targetUserIndex).clase = e_Class.Thief Or UserList(targetUserIndex).clase = e_Class.Pirat Then
...
654             If Not PeleaSegura(UserIndex, targetUserIndex) Then
                ...
                Call SendData(SendTarget.ToPCArea, targetUserIndex, PrepareMessageSetInvisible(UserList(targetUserIndex).Char.charindex, False, UserList(targetUserIndex).pos.x, UserList(targetUserIndex).pos.y))
...
744         b = True
        ...
        If IsFeatureEnabled("healers_and_tanks") And IsSet(Hechizos(h).Effects, e_SpellEffects.ToggleDivineBlood) Then
            If UserList(UserIndex).flags.DivineBlood Then
                ...
                Call ChangeUserChar(UserIndex, Character.Char.body, Character.Char.head, Character.Char.Heading, Character.Char.WeaponAnim, Character.Char.ShieldAnim, Character.Char.CascoAnim, Character.Char.CartAnim, Character.Char.BackpackAnim)
...
Private Sub InfoHechizoDeNpcSobreUser(ByVal NpcIndex As Integer, _
                                      ByVal TargetUser As Integer, _
                                      ByVal Spell As Integer)
...
Sub HechizoPropUsuario(ByVal UserIndex As Integer, _
                       ByRef b As Boolean, _
                       ByRef IsAlive As Boolean)
...
Sub HechizoCombinados(ByVal UserIndex As Integer, _
                      ByRef b As Boolean, _
                      ByRef IsAlive As Boolean)
        '***************************************************
        'Autor: Unknown (orginal version)
...
Private Sub AreaHechizo(UserIndex As Integer, _
...
Private Sub AdjustNpcStatWithCasterLevel(ByVal UserIndex As Integer, _
