...
110             Call WriteLocaleMsg(UserIndex, 77, e_FontTypeNames.FONTTYPE_INFO)
            ...
            If IsSet(Hechizos(HechizoIndex).Effects, e_SpellEffects.CancelActiveEffect) And Hechizos(HechizoIndex).EotId > 0 And IsValidUserRef(UserList(UserIndex).flags.TargetUser) Then
                        ...
                        If .MascotasIndex(j).ArrayIndex > 0 Then
                            If IsValidNpcRef(.MascotasIndex(j)) Then
                                If NpcList(.MascotasIndex(j).ArrayIndex).flags.NPCActive Then
                                    ...
                                    If (Hechizos(h).NumNpc = ELEMENTAL_FUEGO Or Hechizos(h).NumNpc = ELEMENTAL_VIENTO Or Hechizos(h).NumNpc = FUEGOFATUO) And (NpcList(.MascotasIndex(j).ArrayIndex).Numero = ELEMENTAL_FUEGO Or NpcList(.MascotasIndex(j).ArrayIndex).Numero = ELEMENTAL_VIENTO Or NpcList(.MascotasIndex(j).ArrayIndex).Numero = FUEGOFATUO) Then
                                        Index = j
                                        Exit For
...
Public Function ManaHechizoPorClase(ByVal UserIndex As Integer, _
