...
Public Sub SmeltMinerals(ByVal UserIndex As Integer)
    On Error GoTo SmeltMinerals_Err
    With UserList(UserIndex)
        ...
        If .flags.TargetObjInvIndex > 0 Then
            ...
            SkillRequerido = ObjData(.flags.TargetObjInvIndex).MinSkill
            If ObjData(.flags.TargetObjInvIndex).OBJType = e_OBJType.otMinerals And .Stats.UserSkills(e_Skill.Mineria) >= SkillRequerido Then
    ...
    Exit Sub
SmeltMinerals_Err:
    Call TraceError(Err.Number, Err.Description, "Trabajo.SmeltMinerals", Erl)
End Sub
