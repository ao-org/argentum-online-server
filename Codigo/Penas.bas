...
Private Function GlobalChecks(ByVal BannerIndex As Integer, ByRef username As String) As Integer
    ...
    If IsValidUserRef(tUser) Then
        If tUser.ArrayIndex = BannerIndex Then
            Call WriteLocaleMsg(BannerIndex, MSG_NO_PODES_BANEARTE_VOS_MISMO, e_TextChannel.TEXTCHANNEL_SERVER_STAFF, e_FontTypeNames.FONTTYPE_SERVER, vbNullString) ' Msg1841=No podés banearte a vos mismo.
            Exit Function
        End If
        ...
        If CompararUserPrivilegios(tUser.ArrayIndex, BannerIndex) >= 0 Then
            Call WriteLocaleMsg(BannerIndex, MSG_PODES_BANEAR_ALGUIEN_IGUAL_MAYOR_JERARQUIA, e_TextChannel.TEXTCHANNEL_SERVER_STAFF, e_FontTypeNames.FONTTYPE_SERVER) ' Msg2069="No podes banear a al alguien de igual o mayor jerarquia."
            Exit Function
        End If
    Else
        If CompararPrivilegios(UserDarPrivilegioLevel(username), UserList(BannerIndex).flags.Privilegios) >= 0 Then
            Call WriteLocaleMsg(BannerIndex, MSG_PODES_BANEAR_ALGUIEN_IGUAL_MAYOR_JERARQUIA_2070, e_TextChannel.TEXTCHANNEL_SERVER_STAFF, e_FontTypeNames.FONTTYPE_SERVER) ' Msg2070="No podes banear a al alguien de igual o mayor jerarquia."
            Exit Function
        End If
    End If
...
Public Sub BanPJ(ByVal BannerIndex As Integer, ByVal username As String, ByRef Razon As String)
    ...
    If Not PersonajeExiste(username) Then
        Call WriteLocaleMsg(BannerIndex, MSG_PERSONAJE_NO_EXISTE, e_TextChannel.TEXTCHANNEL_SERVER_STAFF, e_FontTypeNames.FONTTYPE_SERVER, vbNullString) ' Msg1842=El personaje no existe.
        Exit Sub
    End If
    If BANCheck(username) Then
        Call WriteLocaleMsg(BannerIndex, MSG_USUARIO_ENCUENTRA_BANEADO, e_TextChannel.TEXTCHANNEL_SERVER_STAFF, e_FontTypeNames.FONTTYPE_WARNING, vbNullString) ' Msg1843=El usuario ya se encuentra baneado.
        Exit Sub
    End If
    ...
    Call SendData(SendTarget.ToAdminsYDioses, 0, PrepareMessageLocaleMsg(MSG_SERVIDOR_BANEADO_DEBIDO_1699, UserList(BannerIndex).name & "¬" & username & "¬" & LCase$(Razon), e_TextChannel.TEXTCHANNEL_SERVER_STAFF, e_FontTypeNames.FONTTYPE_WARNING))  'Msg1699=Servidor » ¬1 ha baneado a ¬2 debido a: ¬3.
...
Public Sub BanPJWithoutGM(ByVal username As String, ByRef Razon As String)
    ...
    ' Le buchoneamos al mundo.
    Call SendData(SendTarget.ToAdminsYDioses, 0, PrepareMessageLocaleMsg(MSG_SERVIDOR_BANEADO_DEBIDO, username & "¬" & LCase$(Razon), e_TextChannel.TEXTCHANNEL_SERVER_STAFF, e_FontTypeNames.FONTTYPE_WARNING))  'Msg1700=Servidor » Ha baneado a ¬1 debido a: ¬2.
    ' Si estaba online, lo echamos.
    Dim tUser As t_UserReference: tUser = NameIndex(username)
    If IsValidUserRef(tUser) Then
        Call WriteDisconnect(tUser.ArrayIndex)
...
Public Sub BanearCuenta(ByVal BannerIndex As Integer, ByVal username As String, ByVal Reason As String)
    ...
    ' Le buchoneamos al mundo.
    Call SendData(SendTarget.ToAdminsYDioses, 0, PrepareMessageLocaleMsg(MSG_SERVIDOR_BANEADO_CUENTA_DEBIDO, UserList(BannerIndex).name & "¬" & username & "¬" & Reason, e_TextChannel.TEXTCHANNEL_SERVER_STAFF, e_FontTypeNames.FONTTYPE_WARNING)) 'Msg1701=Servidor » ¬1 ha baneado la cuenta de ¬2 debido a: ¬3.
    ' Registramos el baneo en los logs.
    ...
    For i = 1 To LastUser
        If UserList(i).AccountID = CuentaID Then
            Call WriteShowMessageBox(i, MSG_HAS_SIDO_BANEADO_DEL_SERVIDOR_MOTIVO, Reason) 'Msg1785=Has sido baneado del servidor. Motivo: ¬1
