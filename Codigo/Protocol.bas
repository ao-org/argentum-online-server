...
Public Function HandleIncomingData(ByVal ConnectionID As Long, _
                                   ByVal Message As Network.reader, _
                                   Optional ByVal optional_user_index As Variant) As Boolean
#Else
...
114             Case Ocultarse
                    ...
                    If Not verifyTimeStamp(PacketCounter, .PacketCounters(PacketNames.Hide), .PacketTimers(PacketNames.Hide), .MacroIterations(PacketNames.Hide), UserIndex, "Ocultar", PacketTimerThreshold(PacketNames.Hide), MacroIterations(PacketNames.Hide)) Then Exit Sub
...
136         If UserList(UserIndex).clase <> QuestList(NpcList(NpcIndex).QuestNumber(Indice)).RequiredClass And QuestList(NpcList(NpcIndex).QuestNumber(Indice)).RequiredClass > 0 Then
