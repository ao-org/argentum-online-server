...
286             Case eSkill.Pescar
                    If .Counters.Trabajando = 0 Then
                        Call Trabajar(UserIndex, eSkill.Pescar)
                    End If
348             Case eSkill.Talar
                    If .Counters.Trabajando = 0 Then
                        Call Trabajar(UserIndex, eSkill.Talar)
                    End If
...
452             Case eSkill.Mineria
                    If .Counters.Trabajando = 0 Then
                        Call Trabajar(UserIndex, eSkill.Mineria)
                    End If
...
110         Call SendData(SendTarget.ToIndex, UserIndex, PrepareMessageChatOverHead("No tengo ninguna misón para ti.", NpcList(NpcIndex).Char.CharIndex, vbWhite))
            Exit Sub
...
144         Call WriteConsoleMsg(UserIndex, "Has aceptado la misón " & Chr(34) & QuestList(.QuestIndex).nombre & Chr(34) & ".", FontTypeNames.FONTTYPE_INFOIAO)
