...
122         If Cantidad > Npclist(NpcIndex).Invent.Object(slot).Amount Then Cantidad = Npclist(UserList(UserIndex).flags.TargetNPC).Invent.Object(slot).Amount
...
134         If MeterItemEnInventario(UserIndex, Objeto) = False Then
                'Call WriteConsoleMsg(UserIndex, "No podés cargar mas objetos.", FontTypeNames.FONTTYPE_INFO)
136             Call EnviarNpcInv(UserIndex, UserList(UserIndex).flags.TargetNPC)
138             Call WriteTradeOK(UserIndex)
                Exit Sub
...
142         Call QuitarNpcInvItem(UserList(UserIndex).flags.TargetNPC, CByte(slot), Cantidad)
...
172             Call EnviarNpcInv(UserIndex, UserList(UserIndex).flags.TargetNPC)
174             Call WriteTradeOK(UserIndex)
                Exit Sub
...
180             Call EnviarNpcInv(UserIndex, UserList(UserIndex).flags.TargetNPC)
182             Call WriteTradeOK(UserIndex)
                Exit Sub
...
188             Call EnviarNpcInv(UserIndex, UserList(UserIndex).flags.TargetNPC)
190             Call WriteTradeOK(UserIndex)
                Exit Sub
...
200             Call EnviarNpcInv(UserIndex, UserList(UserIndex).flags.TargetNPC)
202             Call WriteTradeOK(UserIndex)
                Exit Sub
204         ElseIf ObjData(Objeto.ObjIndex).Real = 1 Then
...
210                 Call EnviarNpcInv(UserIndex, UserList(UserIndex).flags.TargetNPC)
212                 Call WriteTradeOK(UserIndex)
                    Exit Sub
...
218                 Call WriteConsoleMsg(UserIndex, "Las armaduras de la Legión solo pueden ser vendidas a los sastres del demonio.", FontTypeNames.FONTTYPE_INFO)
220                 Call EnviarNpcInv(UserIndex, UserList(UserIndex).flags.TargetNPC)
222                 Call WriteTradeOK(UserIndex)
                    Exit Sub
...
228             Call EnviarNpcInv(UserIndex, UserList(UserIndex).flags.TargetNPC)
                Exit Sub
230         ElseIf UserList(UserIndex).flags.Privilegios And PlayerType.Consejero Then
232             Call WriteConsoleMsg(UserIndex, "No podés vender items.", FontTypeNames.FONTTYPE_WARNING)
234             Call EnviarNpcInv(UserIndex, UserList(UserIndex).flags.TargetNPC)
236             Call WriteTradeOK(UserIndex)
                Exit Sub
...
252             Call EnviarNpcInv(UserIndex, UserList(UserIndex).flags.TargetNPC)
254             Call WriteTradeOK(UserIndex)
...
278     Call UpdateUserInv(False, UserIndex, slot)
280     Call WriteUpdateUserStats(UserIndex)
    
282     Call EnviarNpcInv(UserIndex, UserList(UserIndex).flags.TargetNPC)
284     Call WriteTradeOK(UserIndex)
...
100     Call EnviarNpcInv(UserIndex, UserList(UserIndex).flags.TargetNPC)
...
100     For slot = 1 To UserList(UserIndex).CurrentInventorySlots

102         If Npclist(NpcIndex).Invent.Object(slot).ObjIndex > 0 Then

                Dim thisObj As obj

104             thisObj.ObjIndex = Npclist(NpcIndex).Invent.Object(slot).ObjIndex
106             thisObj.Amount = Npclist(NpcIndex).Invent.Object(slot).Amount
108             val = CLng((ObjData(Npclist(NpcIndex).Invent.Object(slot).ObjIndex).Valor) / Descuento(UserIndex))
        
110             Call WriteChangeNPCInventorySlot(UserIndex, slot, thisObj, val)
            Else

                Dim DummyObj As obj

112             Call WriteChangeNPCInventorySlot(UserIndex, slot, DummyObj, 0)
...
EnviarNpcInv_Err:
        Call RegistrarError(Err.Number, Err.description, "modSistemaComercio.EnviarNpcInv", Erl)
        Resume Next
