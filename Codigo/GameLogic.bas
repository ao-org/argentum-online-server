...
Else
    If PuedeAgua And PuedeTierra Then
        LegalPos = (MapData(Map, x, Y).Blocked = 0) And _
                   ...
                   (PuedeTraslado Or MapData(Map, x, Y).TileExit.Map = 0)

    ElseIf PuedeTierra And Not PuedeAgua Then
        LegalPos = (MapData(Map, x, Y).Blocked = 0) And _
                   (MapData(Map, x, Y).UserIndex = 0) And _
                   (MapData(Map, x, Y).NpcIndex = 0) And _
                   (Not HayAgua(Map, x, Y)) And _
                   (PuedeTraslado Or MapData(Map, x, Y).TileExit.Map = 0)
...
Else
 ...
 If AguaValida = 0 Then
   LegalPosNPC = (MapData(Map, x, Y).Blocked = 0) And _
     (MapData(Map, x, Y).UserIndex = 0) And _
     (MapData(Map, x, Y).NpcIndex = 0) And _
    ...
    If FoundSomething = 1 Then
        ...
        If UserList(UserIndex).Counters.Trabajando = 0 Then
            ...
            Else
                ...
                ElseIf ObjData(UserList(UserIndex).flags.TargetObj).OBJType = eOBJType.otArboles And MapInfo(UserList(UserIndex).Pos.Map).Seguro = False Then
                    Call ActualizarRecurso(Map, UserList(UserIndex).flags.TargetObjX, UserList(UserIndex).flags.TargetObjY)
...
ItemNoEsDeMapa = ObjData(Index).OBJType <> eOBJType.otPuertas And _
            ...
            ObjData(Index).OBJType <> eOBJType.OtCorreo And _
            ObjData(Index).OBJType <> eOBJType.OtDecoraciones
End Function
'[/Barrin 30-11-03]
