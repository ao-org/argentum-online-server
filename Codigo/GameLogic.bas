    ...
    With NpcList(NpcIndex)
        Do
            ...
            For tX = .pos.x - LoopC To .pos.x + LoopC
                If ValidNPCSpawnPos(ClosestLegalPosNPC, .pos.Map, tX, tY, .flags.AguaValida = 1, .flags.TierraInvalida = 0, .flags.PuedeLava = 1, IgnoreUsers, IgnoreDeadUsers) Then
                    Exit Function
                End If
            Next
            tX = .pos.x - LoopC
...
Private Function ValidNPCSpawnPos(OutPos As t_WorldPos, _
    ...
    If LegalPos(Map, x, y, AguaValida, TierraValida, , False) Then
        If SoloLava And Not HayLava(Map, x, y) Then Exit Function
