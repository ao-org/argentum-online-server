...
106             LegalPosNPC = (MapData(Map, x, Y).Blocked = 0) And (MapData(Map, x, Y).Userindex = 0) And (MapData(Map, x, Y).NpcIndex = 0) And (MapData(Map, x, Y).trigger <> eTrigger.POSINVALIDA Or IsPet) And Not HayAgua(Map, x, Y)
            Else
