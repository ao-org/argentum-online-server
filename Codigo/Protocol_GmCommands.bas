                    ...
                    If UCase$(username) <> "YO" Then
                        If .flags.Privilegios And e_PlayerType.Consejero Or e_PlayerType.SemiDios Then
                            If Not EsMapaEvento(Map) Then
                                'Msg1041= Solamente puedes teletransportar gente a mapas de evento.
                                Call WriteLocaleMsg(UserIndex, "1041", e_FontTypeNames.FONTTYPE_INFO)
                                Exit Sub
...
148                     Call SendData(SendTarget.ToGM, 0, PrepareMessageConsoleMsg("Administración » " & .name & " ha silenciado a " & UserList(tUser.ArrayIndex).name & " por " & minutos & " minutos.", e_FontTypeNames.FONTTYPE_GM))
                        'Msg1042= Has sido silenciado por los administradores, no podrás hablar con otros usuarios. Utilice /GM para pedir ayuda.
                        Call WriteLocaleMsg(tUser.ArrayIndex, "1042", e_FontTypeNames.FONTTYPE_GM)
...
156                     Call SendData(SendTarget.ToGM, 0, PrepareMessageConsoleMsg("Administración » " & .name & " ha desilenciado a " & UserList(tUser.ArrayIndex).name & ".", e_FontTypeNames.FONTTYPE_GM))
                        'Msg1043= Has sido desilenciado.
                        Call WriteLocaleMsg(tUser.ArrayIndex, "1043", e_FontTypeNames.FONTTYPE_GM)
...
138             Call LogGM(.name, "/IRA " & username & " Mapa:" & UserList(tUser.ArrayIndex).pos.Map & " X:" & UserList(tUser.ArrayIndex).pos.x & " Y:" & UserList(tUser.ArrayIndex).pos.y)
            Else
                'Msg1044= Servidor » Comando deshabilitado para tu cargo. solo puedes ir a Usuarios que piden SOS.
                Call WriteLocaleMsg(UserIndex, "1044", e_FontTypeNames.FONTTYPE_INFO)
...
102         If (.flags.Privilegios And (e_PlayerType.user Or e_PlayerType.Consejero Or e_PlayerType.RoleMaster Or e_PlayerType.SemiDios)) Then
                'Msg1045= Servidor » /TRABAJANDO es un comando deshabilitado para tu cargo.
                Call WriteLocaleMsg(UserIndex, "1045", e_FontTypeNames.FONTTYPE_INFO)
                Exit Sub
...
120             Call WriteConsoleMsg(UserIndex, "Usuarios trabajando: " & Users, e_FontTypeNames.FONTTYPE_INFO)
            Else
                'Msg1046= No hay usuarios trabajando
                Call WriteLocaleMsg(UserIndex, "1046", e_FontTypeNames.FONTTYPE_INFO)
...
118             Call WriteConsoleMsg(UserIndex, "Usuarios ocultandose: " & Users, e_FontTypeNames.FONTTYPE_INFO)
            Else
                'Msg1047= No hay usuarios ocultandose
...
114             If LenB(username) = 0 Or LenB(Reason) = 0 Then
                    'Msg1048= Utilice /carcel nick@motivo@tiempo
                    Call WriteLocaleMsg(UserIndex, "1048", e_FontTypeNames.FONTTYPE_INFO)
                Else
118                 tUser = NameIndex(username)

120                 If Not IsValidUserRef(tUser) Then
                        'Msg1049= El usuario no está online.
                        Call WriteLocaleMsg(UserIndex, "1049", e_FontTypeNames.FONTTYPE_INFO)
                    Else

124                     If EsGM(tUser.ArrayIndex) Then
                            'Msg1050= No podés encarcelar a administradores.
                            Call WriteLocaleMsg(UserIndex, "1050", e_FontTypeNames.FONTTYPE_INFO)
128                     ElseIf jailTime > 240 Then
                            'Msg1051= No podés encarcelar por más de 4 horas.
                            Call WriteLocaleMsg(UserIndex, "1051", e_FontTypeNames.FONTTYPE_INFO)
                        Else
...
102         If .flags.Privilegios And (e_PlayerType.user Or e_PlayerType.Consejero) Then
                'Msg1052= Solo los Administradores y Dioses pueden usar este comando.
                Call WriteLocaleMsg(UserIndex, "1052", e_FontTypeNames.FONTTYPE_INFO)
                Exit Sub
...
122             Call ReSpawnNpc(auxNPC)
            Else
                'Msg1053= Debes hacer click sobre el NPC antes
                Call WriteLocaleMsg(UserIndex, "1053", e_FontTypeNames.FONTTYPE_INFO)
...
110         If Len(username) = 0 Or Len(Trim$(Reason)) = 0 Then
                'Msg1054= Formato inválido. /advertencia nick@motivo
                Call WriteLocaleMsg(UserIndex, "1054", e_FontTypeNames.FONTTYPE_INFO)
                Exit Sub
...
118                     If .flags.Muerto = 1 Then
                            If UserList(UserIndex).flags.Privilegios And e_PlayerType.SemiDios Then
                                If MapInfo(.pos.Map).Seguro = 0 Or EsMapaEvento(.pos.Map) = False Then
                                    'Msg1055= Servidor » No puedes revivir en una zona insegura.
                                    Call WriteLocaleMsg(UserIndex, "1055", e_FontTypeNames.FONTTYPE_INFO)
                                    Exit Sub
...
122             Call WriteConsoleMsg(UserIndex, list & ".", e_FontTypeNames.FONTTYPE_INFO)
            Else
                'Msg1056= No hay GMs Online.
                Call WriteLocaleMsg(UserIndex, "1056", e_FontTypeNames.FONTTYPE_INFO)
...
102         If Not IsValidNpcRef(.flags.TargetNPC) Then
                'Msg1057= Primero tenés que seleccionar al sacerdote.
                Call WriteLocaleMsg(UserIndex, "1057", e_FontTypeNames.FONTTYPE_INFO)
                Exit Sub
...
110         If Distancia(.pos, priest.pos) > 3 Then
                'Msg1058= El sacerdote no puede escuchar tus pecados debido a que estás demasiado lejos.
                Call WriteLocaleMsg(UserIndex, "1058", e_FontTypeNames.FONTTYPE_INFO)
                Exit Sub
...
124                     ' Msg563=Has desplazado a la criatura.
                    ...
                    Else
                        'Msg1059= Sólo puedes mover NPCs dentro del mismo mapa.
                        Call WriteLocaleMsg(UserIndex, "1059", e_FontTypeNames.FONTTYPE_INFO)
...
136             If NotConsejero Or .pos.Map = UserList(tUser.ArrayIndex).pos.Map Then
                    If .flags.Privilegios And (e_PlayerType.SemiDios) Then
                        If Not EsMapaEvento(.pos.Map) Then 'HarThaos ReyarB hay que sacar eso de mapa seguro y poner mapas de eventos [MapasEventos]
                            'Msg1060= Solamente puedes traer usuarios a mapas de evento.
                            Call WriteLocaleMsg(UserIndex, "1060", e_FontTypeNames.FONTTYPE_INFO)
                            Exit Sub

                        End If

                        If UserList(tUser.ArrayIndex).flags.Muerto = 1 Then
                            'Msg1061= No puedes transportar a un muerto. ellos tienen /hogar
                            Call WriteLocaleMsg(UserIndex, "1061", e_FontTypeNames.FONTTYPE_INFO)
                            Exit Sub
...
138                 If NotConsejero And .flags.AdminInvisible = 1 Then
                        'Msg1062= Te han trasportado.
                        Call WriteLocaleMsg(tUser.ArrayIndex, "1062", e_FontTypeNames.FONTTYPE_INFO)
                    Else
...
108         ElseIf .flags.Privilegios And (e_PlayerType.SemiDios) Then
                'Msg1063= Servidor » La cantidad de NPCs disponible para tu rango está limitada.
                Call WriteLocaleMsg(UserIndex, "1063", e_FontTypeNames.FONTTYPE_INFO)
...
104             'Msg528=Servidor » Comando deshabilitado para tu cargo.
            ...
            'Msg1064= Se han limpiado los items del suelo.
            Call WriteLocaleMsg(UserIndex, "1064", e_FontTypeNames.FONTTYPE_INFO)
...
138                     Call WriteConsoleMsg(UserIndex, "Los personajes con ip " & IP & " son: " & lista, e_FontTypeNames.FONTTYPE_INFO)
                ...
                Else
                    'Msg1065= No hay ningun personaje con ese nick
                    Call WriteLocaleMsg(UserIndex, "1065", e_FontTypeNames.FONTTYPE_INFO)
...
120         If MapData(Mapa, x, y).ObjInfo.ObjIndex > 0 Then
                'Msg1066= Hay un objeto en el piso en ese lugar
                Call WriteLocaleMsg(UserIndex, "1066", e_FontTypeNames.FONTTYPE_INFO)
                Exit Sub

            End If

124         If MapData(Mapa, x, y).TileExit.Map > 0 Then
                'Msg1067= No podés crear un teleport que apunte a la entrada de otro.
...
110                 UserList(tUser.ArrayIndex).DescRM = Desc
                Else
                    'Msg1068= Haz click sobre un personaje antes!
                    Call WriteLocaleMsg(UserIndex, "1068", e_FontTypeNames.FONTTYPE_INFO)
...
108                 Call SendData(SendTarget.ToNPCArea, .flags.TargetNPC.ArrayIndex, PrepareMessageChatOverHead(Message, NpcList(.flags.TargetNPC.ArrayIndex).Char.charindex, vbWhite))
                Else
                    'Msg1069= Debes seleccionar el NPC por el que quieres hablar antes de usar este comando
...
106         If Not EsGM(UserIndex) Or (.flags.Privilegios And (e_PlayerType.user Or e_PlayerType.Consejero Or e_PlayerType.SemiDios Or e_PlayerType.Dios)) Then
                'Msg1072= Comando deshabilitado para tu cargo.
...
126                 Call WriteConsoleMsg(UserIndex, "Has creado " & Objeto.amount & " unidades de " & ObjData(tObj).name & ".", e_FontTypeNames.FONTTYPE_INFO)
                Else
                    'Msg1073= No tenes espacio en tu inventario para crear el item.
...
110             If LenB(username) = 0 Then
                    'Msg1074= Utilice /borrarpena Nick@NumeroDePena@NuevaPena
...
122                 Call LogGM(.name, "/LASTIP " & username)
                ...
                ip_list = Split(ipStr, ";")
                'Msg1076= Las últimas ips para el personaje son:
...
122                 Next LoopC
                    ...
                    'Msg1077= Se ha cambiado el MOTD con exito
...
104         If .flags.Privilegios And (e_PlayerType.Consejero Or e_PlayerType.SemiDios Or e_PlayerType.Dios) Then
                'Msg1078= Servidor » Comando deshabilitado para tu cargo. Si el motivo es probar algo ya saben ir a Test
...
Public Sub HandleAlterName(ByVal UserIndex As Integer)
        ...
        With UserList(UserIndex)
            ...
            If LenB(username) = 0 Or LenB(NewName) = 0 Then
                'Msg1079= Usar: /ANAME origen@destino
            ...
            If PersonajeExiste(NewName) Then
                'Msg1080= El nick solicitado ya existe.
...
146                 Case Else
                        'Msg1082= Opciones para restringir: 'NEWBIE', 'SINMAGIA', 'SININVI', 'NOPKS', 'NOCIUD'
...
114                 Call WriteConsoleMsg(UserIndex, "Mapa " & .pos.Map & " Terreno: " & MapInfo(.pos.Map).terrain, e_FontTypeNames.FONTTYPE_INFO)
                Else
                    'Msg1083= Opciones para terreno: 'BOSQUE', 'NIEVE', 'DESIERTO', 'CIUDAD', 'CAMPO', 'DUNGEON'
                    Call WriteLocaleMsg(UserIndex, "1083", e_FontTypeNames.FONTTYPE_INFO)
                    'Msg1084= Igualmente, el ínico ítil es 'NIEVE' ya que al ingresarlo, la gente muere de frio en el Mapa
                    Call WriteLocaleMsg(UserIndex, "1084", e_FontTypeNames.FONTTYPE_INFO)
...
114                 Call WriteConsoleMsg(UserIndex, "Mapa " & .pos.Map & " Zona: " & MapInfo(.pos.Map).zone, e_FontTypeNames.FONTTYPE_INFO)
                Else
                    'Msg1085= Opciones para terreno: 'BOSQUE', 'NIEVE', 'DESIERTO', 'CIUDAD', 'CAMPO', 'DUNGEON'
                    Call WriteLocaleMsg(UserIndex, "1085", e_FontTypeNames.FONTTYPE_INFO)
                    'Msg1086= Igualmente, el ínico ítil es 'DUNGEON' ya que al ingresarlo, NO se sentirí el efecto de la lluvia en este mapa.
                    Call WriteLocaleMsg(UserIndex, "1086", e_FontTypeNames.FONTTYPE_INFO)
...
110                     Call LogGM(.name, .name & " ha cambiado la configuracion el dropeo de items en el mapa" & UserList(UserIndex).pos.Map & " a " & MapInfo(UserList(UserIndex).pos.Map).DropItems)
                        'Msg1087= Mapa actualizado correctamente
...
118                     Call LogGM(.name, .name & " ha cambiado la configuracion el pelea segura del mapa" & UserList(UserIndex).pos.Map & " a " & MapInfo(UserList(UserIndex).pos.Map).DropItems)
                        'Msg1088= Mapa actualizado correctamente
...
124                     Call LogGM(.name, .name & " ha cambiado la configuracion el friendly fire del mapa" & UserList(UserIndex).pos.Map & " a " & MapInfo(UserList(UserIndex).pos.Map).DropItems)
                        'Msg1089= Mapa actualizado correctamente
                        Call WriteLocaleMsg(UserIndex, "1089", e_FontTypeNames.FONTTYPE_INFO)

                    Case Else
                        'Msg1090= Opcion no disponible
                        Call WriteLocaleMsg(UserIndex, "1090", e_FontTypeNames.FONTTYPE_INFO)
...
106         Call CargaNpcsDat
            'Msg1091= Npcs.dat recargado exitosamente.
            Call WriteLocaleMsg(UserIndex, "1091", e_FontTypeNames.FONTTYPE_SERVER)
...
110         Call LoadRecursosEspeciales
            'Msg1092= Obj.dat recargado exitosamente.
            Call WriteLocaleMsg(UserIndex, "1092", e_FontTypeNames.FONTTYPE_SERVER)
...
118                     Call WriteConsoleMsg(UserIndex, " Objeto " & Slot & ") " & ObjData(UserList(tUser.ArrayIndex).invent.Object(Slot).ObjIndex).name & " Cantidad:" & UserList(tUser.ArrayIndex).invent.Object(Slot).amount, e_FontTypeNames.FONTTYPE_INFO)
                    Else
                        'Msg1093= No hay Objeto en slot seleccionado
                        Call WriteLocaleMsg(UserIndex, "1093", e_FontTypeNames.FONTTYPE_INFO)

                    End If

                Else
                    'Msg1094= Slot Invílido.
                    Call WriteLocaleMsg(UserIndex, "1094", e_FontTypeNames.FONTTYPE_TALK)
...
132                 Call SendData(SendTarget.ToAll, 0, PrepareMessageConsoleMsg(.name & " ha otorgado a " & UserList(tUser.ArrayIndex).name & " " & Cantidad & " " & ObjData(ObjIndex).name & ": " & Motivo, e_FontTypeNames.FONTTYPE_ROSA))
                Else
134                 ' Msg550=El usuario no tiene espacio en el inventario.
                    Call WriteLocaleMsg(UserIndex, "550", e_FontTypeNames.FONTTYPE_INFO)
...
108             Call FinalizarEvento
            Else
                'Msg1095= No hay ningun evento activo.
                Call WriteLocaleMsg(UserIndex, "1095", e_FontTypeNames.FONTTYPE_New_Eventos)
...
Public Sub HandleCancelarEvento(ByVal UserIndex As Integer)
        ...
        If CurrentActiveEventType = CaptureTheFlag Then
            If InstanciaCaptura Is Nothing Then
                'Msg1096= Eventos » No hay ninguna instancia en curso para ese evento.
                Call WriteLocaleMsg(UserIndex, "1096", e_FontTypeNames.FONTTYPE_INFO)
            Else
...
108                 tUser = NameIndex(username)
                Else
                    'Msg1097= No puedes seguirte a vos mismo
                    Call WriteLocaleMsg(UserIndex, "1097", e_FontTypeNames.FONTTYPE_INFO)
                    Exit Sub
...
104             If Not HandleRemoteLobbyCommand(Command, Params, UserIndex, GlobalLobbyIndex) Then
                    'Msg1098= Servidor » No se pudo procesar el comando.
                    Call WriteLocaleMsg(UserIndex, "1098", e_FontTypeNames.FONTTYPE_INFO)
...
Public Sub HandleFeatureToggle(ByVal UserIndex As Integer)
        ...
        If (UserList(UserIndex).flags.Privilegios And (e_PlayerType.Admin)) Then
            ...
            'Msg1099= variable configurada correctamente.
            Call WriteLocaleMsg(UserIndex, "1099", e_FontTypeNames.FONTTYPE_INFO)
        Else
            'Msg1100= no tienes permisos para realizar esta accion.
            Call WriteLocaleMsg(UserIndex, "1100", e_FontTypeNames.FONTTYPE_INFO)
...
Public Sub HandleIniciarCaptura(ByVal UserIndex As Integer, _
                                EventSettings As t_NewScenearioSettings)
...
104         If (.flags.Privilegios And (e_PlayerType.Admin Or e_PlayerType.Dios Or e_PlayerType.SemiDios)) Then
                If Not InstanciaCaptura Is Nothing Then
                    'Msg1101= Ya hay un evento de captura de bandera en curso.
                    Call WriteLocaleMsg(UserIndex, "1101", e_FontTypeNames.FONTTYPE_INFO)
                    Exit Sub
                Else

                    'El precio no puede ser negativo
                    If EventSettings.InscriptionFee < 0 Then
                        'Msg1102= El valor de la entrada al evento no podrá ser menor que 0.
                        Call WriteLocaleMsg(UserIndex, "1102", e_FontTypeNames.FONTTYPE_INFO)
                        Exit Sub
                    ...
                    If EventSettings.MaxPlayers Mod 2 <> 0 Then
                        'Msg1103= La cantidad de participantes debe ser un número par.
                        Call WriteLocaleMsg(UserIndex, "1103", e_FontTypeNames.FONTTYPE_INFO)
                        Exit Sub
                    ...
                    If EventSettings.MaxPlayers > 48 Then 'Leer de una variable de configuración
                        'Msg1104= La cantidad de participantes no podrá ser mayor que 48.
                        Call WriteLocaleMsg(UserIndex, "1104", e_FontTypeNames.FONTTYPE_INFO)
                        Exit Sub
                    ...
                    If EventSettings.MaxPlayers > NumUsers Then
                        'Msg1105= Hay pocos jugadores en el servidor, intenta con una cantidad menor de participantes.
                        Call WriteLocaleMsg(UserIndex, "1105", e_FontTypeNames.FONTTYPE_INFO)
                        Exit Sub

                    End If

                    If EventSettings.MinLevel < 1 Or EventSettings.MinLevel > 47 Then
                        'Msg1106= El nivel para el evento debe ser entre 1 y 47.
                    ...
                    If EventSettings.MinLevel > EventSettings.MaxLevel Then
                        'Msg1107= El nivel minimo debe ser menor al maximo.
                        Call WriteLocaleMsg(UserIndex, "1107", e_FontTypeNames.FONTTYPE_INFO)
                        Exit Sub
...
Private Sub HandleStartGenericLobby(ByVal UserIndex As Integer, _
                                    ByRef LobbySettings As t_NewScenearioSettings)

        On Error GoTo ErrHandler

        If IsEventActive Then
            'Msg1108= Ya hay un evento activo, debes cancelarlo primero.
            Call WriteLocaleMsg(UserIndex, "1108", e_FontTypeNames.FONTTYPE_INFO)
            Exit Sub
        ...
        If GlobalLobbyIndex < 0 Then
            'Msg1109= No se pudo encontrar una sala activa.
            Call WriteLocaleMsg(UserIndex, "1109", e_FontTypeNames.FONTTYPE_INFO)
            Exit Sub
...
136             'Me fijo si hay más participantes conectados que el cupo para jugar
                ...
                'Msg1110= Se creo el lobby, recorda que tenes que abrirlo para que se pueda anotar gente.
                Call WriteLocaleMsg(UserIndex, "1110", e_FontTypeNames.FONTTYPE_INFO)
