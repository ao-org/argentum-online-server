...
The project includes a built-in unit test suite that validates core server modules.
...
**Still deferred (high effort):**
- `GetClassAttackModifier` / `GetUserDamageWithItem` in SistemaCombate.bas — need class data and full user+item setup from .dat files
- Additional `ValidDescription` edge cases (empty string, single char boundaries)
